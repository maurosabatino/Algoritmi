...
import sabatino.esercizio13.searchTree.Elem;
...
public class BinSearchTreeGUI <V,K extends Comparable<K>> extends JFrame {
  private static final long serialVersionUID = 1L;
  private SearchTree<K> tree;
  ...
  public BinSearchTreeGUI(String title) {
    ...
    searchButton.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          int key = Integer.parseInt(text2.getText());
          Elem elem = (Elem) tree.find(key);
          if(elem == null) text1.setText("chiave non presente");
          else text1.setText(elem.nome);
          repaint();
        }
      }
    );
    ...
    if(option == APPROVE_OPTION) {
      ...
      try {
        ...
        while(input.hasNextLine()) {
          ...
          String nome = (String)input.nextLine().trim();
          tree.put(new Elem<K>(nome, matr));
        }
        input.close();
      }
